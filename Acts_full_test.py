...
from Open.spec_project_new_separ import get_blocks
...
#Функция выбора нового типа спецификации проекта

def new_spec_changed():
    info.new_spec_chooced(new_spec_choice.get())

# функция выбора разбора спецификации нового образца по блочно

def to_blocks_changed():
    new_spec_choice.set(1)
    info.to_blocks_chooced(to_blocks.get())
...
def safe_act_project(event):
    ...
    if to_blocks.get()==1:
        context['station'] =combo_block.get()
    else:
        context['station']=project_frame.station.get()
    ...
    if filepath != "" and len(lst_xl)>1:
        ...
        #Проверяем поблочно или нет
        if to_blocks.get()==1:
            new_table = fill_table_project(project_frame.type_choies.get(), cols_number, lst_xl_list[names_blocks.index(combo_block.get())], new_table)
        else:
...
def open_table_project(event):
    ...
    if new_spec_choice.get()==1 and to_blocks.get()!=1:
        lst_xl=spec_parce_new(table_path)
                    
    elif to_blocks.get()==1:

        names_blocks=get_blocks(table_path)
        combo_block.set(names_blocks[0])

        # Текстовая метка
        name_blocks = Label(project_frame, text='Выберите блок', font=("Arial", 11, "bold"))
        name_blocks.place(x=390, y=350)
        # Добавляем во вкладку проекта ниспадающий список для выбора блока для которого будут готовиться акты
        # Ниспадающий список
        block_choies_combobox = Combobox(project_frame, textvariable=combo_block, values=names_blocks, state="readonly")
        block_choies_combobox.place(x=390, y=380, width=150)

        lst_xl = spec_parce_new(table_path)
        for i in range(len(names_blocks)):
            lst_xl_list.append(spec_parce_new_separ(table_path)[i])
...
# Доабавляем во вкладку спецификация проекта чекбокс для выбора обработки спецификации нового образца
new_spec_choice = IntVar()

new_spec_checkbutton = ttk.Checkbutton(project_frame, text="Спецификация новая", variable=new_spec_choice,
                                       command=new_spec_changed)

new_spec_checkbutton.place(x=560, y=400)

# Доабавляем во вкладку спецификация проекта чекбокс для выбора обработки спецификации нового образца поблочно
to_blocks = IntVar()

to_blocks_checkbutton = ttk.Checkbutton(project_frame, text="Поблочно", variable=to_blocks,
                                       command=to_blocks_changed)

to_blocks_checkbutton.place(x=560, y=430)
...
# создаем переменную флаг для выбора пути сохранения всех актов сразу
all_safe_enabled=0
# создаем пустой список для получения его из спецификации проекта
lst_xl=[]
# создаем пустой список для получения его из спецификации проекта поблочно
lst_xl_list=[]
# создаем директорию пути сохранения акта
dir_path=''
# создаем пустой список для труб
tube_list=[]
# для прокладок
pad_list=[]
# для опор
support_list=[]
# для элементов трубопроводов
elements_list=[]
# создаем пустой список для названия блоков
names_blocks=[]
# по умолчанию будет выбран первый элемент из names_blocks
combo_block = StringVar()
